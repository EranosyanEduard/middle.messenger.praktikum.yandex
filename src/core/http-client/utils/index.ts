...
/**
 * "Собрать" URL-адрес на основании его частей, переданных в аргументе [urlDetails].
 * @param urlDetails объект, содержащий следующие части URL-адреса:
 * base - адрес сервера; entryPoint - конечная точка; queryString - параметры запроса;
 */
function joinURL(urlDetails: TUrlDetails): string {
    // Создать копию объекта urlDetails, очистив значения объекта от "висячих" слешов.
    const {base, entryPoint, queryParams} = Object.entries(urlDetails).reduce<TRecord<string>>((acc, url) => {
        const [type, val] = url
